class AgentsController < ApplicationController
  ...
  def breakdown_pdf
    ...
    @start = Occurrence.joins(:entries).where("entries.agent_id = ?", @agent.id).where('entries.edate' => 60.days.ago...Date.today.beginning_of_day).order('entries.edate DESC').select(:'entries.edate', :ovalue).group_by { |b| b.edate.strftime("%B") }
