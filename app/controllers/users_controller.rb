class UsersController < ApplicationController
  ...
  def dashboard          
    ...
    @agent_chart_labels_effective = Incentive.all.where(:date => 2.months.ago.to_date..Date.today.to_date.end_of_day).where("user_id = ?", helpers.current_user.id).map { |l| l.date.strftime("%Y-%m-%d") }.to_a;
  	@agent_chart_data_effective = Incentive.all.where(:date => 2.months.ago.to_date..Date.today.to_date.end_of_day).where("user_id = ?", helpers.current_user.id).map { |l| l.uph }.to_a;
    @agent_chart_data_effective_occupancy = Incentive.all.where(:date => 2.months.ago.to_date..Date.today.to_date.end_of_day).where("user_id = ?", helpers.current_user.id).map { |l| l.occupancy }.to_a;
