  ...
  def dashboard
    ...
    if current_user.has_role?(:executive) then
      #Dan/dad/rick
           ...
           @agent_occurrence_values = Entry.effective.joins(:occurrence, agent: [:user, :department]).group("users.first_name, users.last_name").order("users.first_name, users.last_name").sum(:ovalue).values
           @agent_occurrence_labels = Entry.effective.joins(:occurrence, agent: [:user, :department]).group("users.first_name, users.last_name").order("users.first_name, users.last_name").pluck("users.first_name, users.last_name")
